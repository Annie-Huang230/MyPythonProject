...
def get_best_pixel(pixels):
    ...
    # Loop through all pixels to find the one closest to the average
    for pixel in pixels:
...
def solve(images):
    ...
    for x in range(width):
        for y in range(height):
            # Collect pixels from all images at (x, y) position
            pixels = []
            for image in images:
                pixels.append(image.get_pixel(x, y))

                # Get the best pixel (closest to the average)
                best_pixel = get_best_pixel(pixels)

                # Set the result pixel's color to the best pixel's color
